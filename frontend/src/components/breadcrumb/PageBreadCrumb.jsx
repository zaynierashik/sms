				...
				<ol className="flex items-center gap-1.5 flex-wrap">
					{pathSegments.map((segment, index) => (
						<li key={index} className="flex items-center gap-1.5 text-sm font-normal">
							{segment.path ? (
								<Link to={segment.path} className="text-gray-500 dark:text-gray-400 inline-flex items-center gap-1.5">
									{segment.name}
								...
								<span className="text-gray-800 dark:text-white/90">{segment.name}</span>
							)}
						</li>
					))}
				</ol>
